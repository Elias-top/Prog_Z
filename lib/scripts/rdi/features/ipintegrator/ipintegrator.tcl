...
# libraries
rdi::load_library ipservices librdi_ipservicestasks
rdi::load_library systembuilder librdi_rsbtasks

# don't autocomplete bd namespace
rdi::hide_namespaces bd::

rdi::hide_commands auto_bd_connect  generate_bd_design get_bd_files

proc get_bd_files { design_name } {
   set bdj_file [get_files -norecurse -quiet ${design_name}.bdj -all]
   if { $bdj_file ne "" } {
      return $bdj_file
   }

   set bd_file [get_files -norecurse -quiet ${design_name}.bd -all]
   return $bd_file
}
...
namespace eval bd {
   ...
   proc get_reg_of_name {seg reg_name} {
   ...
   proc mark_propagate_overrideable { cell_name paramList } {
      ...
      set listParamMode ""
      foreach param $paramList {
         set suffix ".VALUE_MODE"   
         set pName CONFIG.$param$suffix
         set pVal "AUTO"
         set oldMode [get_property $pName [get_bd_cells $cell_name] -quiet] 
         if {$oldMode == "none"} {
              lappend listParamMode "$pName"
              lappend listParamMode "$pVal"
         }
      }
         set_property -dict "$listParamMode" [get_bd_cells $cell_name] -quiet
   ...
   proc get_bd_ext { } {
      set bd_ext "bd"
      #set bd_ext [::bd::util_cmd get_bd_ext]

      return $bd_ext
   }

   # Get a file from RDI_DATADIR.  This may be a list.
   proc ::bd::utils::get_rdi_datadir_file {filepath} {

      if { [info exist ::env(RDI_DATADIR)] == 0 } {
         return $filepath
      }

      set datadir $::env(RDI_DATADIR)
      set paths [split $datadir ":"]

      foreach path $paths {
         if { [file exists $path/$filepath] == 1 } {
