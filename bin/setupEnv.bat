...
if not defined HW_SERVER_FTDIMGR_DISABLE (
  set HW_SERVER_FTDIMGR_DISABLE=1
)

if not defined HW_SERVER_DJTG_DISABLE (
  set HW_SERVER_DJTG_DISABLE=1
)
...
rem #
rem # Calculate the length of %PYTHONPATH% set and if greater than 519 characters, throw warning.
rem # CR 1050435
rem #
set python_path=%PYTHONPATH%
if defined python_path if not "%python_path:~519,1%"=="" (
  echo !! WARNING !!
  echo "PYTHONPATH" variable is set greater than default max string limit of "519" characters.
  echo Please ignore this, if you have already made changes to your system registry to accept more than
  echo default limit of 519 characters.
  echo !! WARNING !!
)

set session_string=%cd%:%COMPUTERNAME%-%DATE:/=-%_%TIME::=-%
set session_string=%session_string: =%
if not defined RDI_SESSION_INFO (
  set RDI_SESSION_INFO=%session_string%
)
